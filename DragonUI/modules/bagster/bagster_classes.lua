...
do
    ...
    function ItemSlot:New()
        ...
        local bagsConfig = DragonUI and DragonUI.db and DragonUI.db.profile and DragonUI.db.profile.bags
...
do
    ...
    function ItemFrame:HasItem(bag, slot, link)
        ...
        if next(f) then
            ...
            -- GetItemInfo order: name, link, quality, itemLevel, reqLevel, itemType, subType, stackCount, equipLoc
            local name, quality, itemLevel, reqLevel, itemType, subType, stackCount, equipLoc
            if link then
                name, link, quality, itemLevel, reqLevel, itemType, subType, stackCount, equipLoc = GetItemInfo(link)
            end

            if f.quality and f.quality > 0 and not (quality and bit.band(f.quality, mod.QualityFlags[quality] or 0) > 0) then
                return false
            elseif f.rule and not f.rule(player, bagType, name, link, quality, itemLevel, reqLevel, itemType, subType, stackCount, equipLoc) then
                return false
            elseif f.subRule and not f.subRule(player, bagType, name, link, quality, itemLevel, reqLevel, itemType, subType, stackCount, equipLoc) then
                return false
            end
        end
        return true
