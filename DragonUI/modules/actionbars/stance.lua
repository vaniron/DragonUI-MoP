...
local NUM_SHAPESHIFT_SLOTS = NUM_STANCE_SLOTS or 10;
...
local function stancebutton_update()
    ...
    _G.StanceButton1:SetPoint('BOTTOMLEFT', anchor, 'BOTTOMLEFT', 0, 0)
end
    ...
    for index=1, NUM_SHAPESHIFT_SLOTS do
		...
		if button then
		    ...
		    if index == 1 then
			    ...
			    local previous = _G['StanceButton'..index-1]
...
local function ApplyStanceSystem()
    ...
    if not StanceModule.hooks.StanceBar_Update then
        StanceModule.hooks.StanceBar_Update = true
...
local function RestoreStanceSystem()
    ...
    for index=1, NUM_SHAPESHIFT_SLOTS do
        local button = _G['StanceButton'..index]
        if button then
            button:SetParent(StanceBarFrame or UIParent)
            button:ClearAllPoints()
