...
local function ReplaceBlizzardFrame(frame)
    ...
    -- Player arrow size (configurable; methods removed in MoP 5.4.8)
    ...
    if minimapFrame.SetPlayerTextureWidth then
        minimapFrame:SetPlayerTextureWidth(playerArrowSize)
    end

    -- Blip texture (configurable: new DragonUI icons vs old Blizzard icons)
    -- Default to Blizzard's ObjectIcons because DragonUI's custom atlas does not
    -- include MoP quest POI icons (question marks / exclamation marks), which
    -- would otherwise render as geometric shapes.
    ...
    if useNewBlipStyle == nil then
        useNewBlipStyle = false -- Default to Blizzard native style for MoP compatibility
    end
...
local BLIZZARD_MINIMAP_BUTTONS = {
    ...
    ['QueueStatusMinimapButton'] = true, -- MoP 5.4.8 unified queue button (BG/LFG)
...
local function StylePVPBattlefieldFrame()
    ...
    -- Ensure textures exist before applying atlas (SetNormalTexture('') removes them)
    local normalTexture = MiniMapBattlefieldFrame:GetNormalTexture() or MiniMapBattlefieldFrame:CreateTexture(nil, "ARTWORK")
    normalTexture:set_atlas('Minimap-PVP-' .. faction .. '-Normal', true)
    MiniMapBattlefieldFrame:SetNormalTexture(normalTexture)

    local pushedTexture = MiniMapBattlefieldFrame:GetPushedTexture() or MiniMapBattlefieldFrame:CreateTexture(nil, "ARTWORK")
    pushedTexture:set_atlas('Minimap-PVP-' .. faction .. '-Pushed', true)
    MiniMapBattlefieldFrame:SetPushedTexture(pushedTexture)
...
-- Position the holder relative to the minimap. Uses the original SetPoint /
-- ClearAllPoints methods so it works even while the holder's methods are locked
-- against external movement. Recomputed on every call because the minimap can
-- change (e.g. entering a battleground), which used to leave the eye half-hidden
-- behind the minimap.
local function RepositionLFGHolder(holder)
    if not holder then return end

    local setPoint = holder.DragonUI_OrigSetPoint or holder.SetPoint
    local clearPoints = holder.DragonUI_OrigClearAllPoints or holder.ClearAllPoints
    clearPoints(holder)

    if MinimapModule.minimapFrame then
        local frameW = MinimapModule.minimapFrame:GetWidth()
        local frameH = MinimapModule.minimapFrame:GetHeight()
        local mapW = Minimap:GetWidth() * Minimap:GetScale()
        local mapH = Minimap:GetHeight() * Minimap:GetScale()
        local mapCenterX = frameW / 2
        local mapCenterY = frameH / 2 - 25
        local mapLeft = mapCenterX - mapW / 2
        local mapBottom = mapCenterY - mapH / 2
        local targetLeft = 45
        local targetBottom = 10
        setPoint(holder, "BOTTOMLEFT", Minimap, "BOTTOMLEFT",
            targetLeft - mapLeft, targetBottom - mapBottom)
    else
        setPoint(holder, "BOTTOMLEFT", Minimap, "BOTTOMLEFT", 2, 2)
    end

    if WorldMapFrame and WorldMapFrame:IsShown() then
        holder:SetFrameStrata("FULLSCREEN_DIALOG")
        holder:SetFrameLevel(25)
    else
        holder:SetFrameStrata(Minimap:GetFrameStrata())
        holder:SetFrameLevel(Minimap:GetFrameLevel() + 25)
    end
end

-- Ensure the visual holder exists. It is created on demand so it survives
-- even when QueueStatusMinimapButton does not exist at login.
local function GetOrCreateLFGHolder()
    local holderName = "DragonUI_LFGHolder"
    ...
    -- Lock the holder in place so external code cannot move it.
    holder.DragonUI_OrigSetPoint = holder.SetPoint
    holder.DragonUI_OrigClearAllPoints = holder.ClearAllPoints
    holder.SetPoint = function(self, ...) end
    holder.ClearAllPoints = function(self) end

    local ring = holder:CreateTexture(nil, "OVERLAY")
    ring:SetTexture("Interface\\AddOns\\DragonUI\\Textures\\Minimap\\border_buttons.tga")
    ...
    RepositionLFGHolder(holder)

    return holder
end

-- MoP 5.4.8 unified queue button (BG + LFG). Classic MiniMapBattlefieldFrame may not exist.
local function StyleQueueStatusButton()
    ...
    -- Only style when the eye is actually shown by Blizzard (i.e. when queued).
    -- Do not force Show(); the native state must remain invisible when not queued.
    if button.SetAlpha then button:SetAlpha(1) end
    local eye = button.Eye or button.Icon or QueueStatusMinimapButtonIcon
    if eye and eye:IsShown() then
        if eye.SetAlpha then eye:SetAlpha(1) end
        -- Scale the eye up so it fills the golden ring. The native eye is
        -- smaller than the button frame, so 1.5 makes it match the 25x25 ring.
        if eye.SetScale then eye:SetScale(1.5) end
    end
    local border = button.Border or QueueStatusMinimapButtonBorder
    if border and border.SetTexture then
        border:SetTexture(nil)
    ...
    -- Do NOT overwrite the native OnClick. The default MoP handler shows the
    -- queue menu (estimated time, leave queue, enter dungeon, etc.) and changes
    -- behaviour depending on whether the player is queued, in a dungeon, or in a BG.

    -- Keep the visual holder in sync with the native button's Show/Hide state.
    -- The OnHide hook that used to hide the holder has been removed, so the
    -- holder stays visible even when the world map opens and hides the native
    -- minimap/button parent chain.
    local holder = GetOrCreateLFGHolder()
    holder:SetShown(button:IsShown())
    if button:IsShown() and button.Show then
        button:Show()
    end

    -- In battlegrounds Blizzard reparents/repositions QueueStatusMinimapButton.
    -- Recompute the holder position and re-attach the button so the eye doesn't
    -- end up half-hidden behind the minimap and unclickable.
    RepositionLFGHolder(holder)
    if button:GetParent() ~= holder then
        button:SetParent(holder)
    end
    button.DragonUI_Locked = false
    button:ClearAllPoints()
    button:SetPoint("CENTER", holder, "CENTER", 0, 0)
    button.DragonUI_Locked = true

    -- Hook QueueStatusFrame_Update so visibility/style is refreshed whenever the queue state changes
    if not MinimapModule.queueStatusHooked and QueueStatusFrame_Update then
        hooksecurefunc("QueueStatusFrame_Update", StyleQueueStatusButton)
        MinimapModule.queueStatusHooked = true
    end
end
...
local function RemoveBlizzardFrames()
    ...
    -- MoP creates QueueStatusMinimapButton dynamically when the player queues.
    -- Listen for queue/battleground events so we can style it the moment it appears.
    if not MinimapModule.queueStatusEventFrame then
        MinimapModule.queueStatusEventFrame = CreateFrame("Frame")
        MinimapModule.queueStatusEventFrame:RegisterEvent("LFG_QUEUE_STATUS_UPDATE")
        MinimapModule.queueStatusEventFrame:RegisterEvent("UPDATE_BATTLEFIELD_STATUS")
        MinimapModule.queueStatusEventFrame:RegisterEvent("PLAYER_ENTERING_WORLD")
        MinimapModule.queueStatusEventFrame:RegisterEvent("WORLD_MAP_UPDATE")
        MinimapModule.queueStatusEventFrame:SetScript("OnEvent", function(self, event)
...
-- Get the active tracking texture. WotLK uses GetTrackingTexture(); MoP uses
-- GetTrackingInfo because tracking became multi-select in 4.0.1.
local function GetActiveTrackingTexture()
    if GetTrackingTexture then
        return GetTrackingTexture()
    end
    if GetNumTrackingTypes and GetTrackingInfo then
        for i = 1, GetNumTrackingTypes() do
            local name, tex, active = GetTrackingInfo(i)
            if active then
                return tex
            end
        end
...
function MinimapModule:UpdateTrackingIcon()
    ...
    local texture = GetActiveTrackingTexture()
    ...
    if useOldStyle then
        ...
        if not texture or texture == 'Interface\\Minimap\\Tracking\\None' then
...
-- Dungeon Eye: anchor the LFG/Battlefield queue eye directly to the minimap.
-- The eye must stay attached to the minimap, be invisible when not queued,
-- and retain its native OnClick behaviour (see StyleQueueStatusButton).
function MinimapModule:RegisterLFGEditorFrame()
    ...
    if not lfgFrame.DragonUI_OriginalParent then
        lfgFrame.DragonUI_OriginalParent = lfgFrame:GetParent()
    end
    if not lfgFrame.DragonUI_OriginalScale then
        lfgFrame.DragonUI_OriginalScale = lfgFrame:GetScale()
    end
    if not lfgFrame.DragonUI_OriginalPoints then
        lfgFrame.DragonUI_OriginalPoints = {}
        for i = 1, lfgFrame:GetNumPoints() do
            local point, relativeTo, relativePoint, x, y = lfgFrame:GetPoint(i)
            table.insert(lfgFrame.DragonUI_OriginalPoints,
    ...
    -- Move the native LFG button inside the holder and scale it to 21x21.
    lfgFrame:SetParent(holder)
    lfgFrame:ClearAllPoints()
    lfgFrame:SetPoint("CENTER", holder, "CENTER", 0, 0)

    -- Do NOT hook OnHide to hide the holder. When the world map opens, Blizzard
    -- may hide the native button/minimap parent chain; we want the holder to
    -- remain visible based on the player's queue state instead.

    -- Calculate scale so the native button renders at the same 21x21 size as
    -- DragonUI's minimap settings button. Store native size on first run only.
    if not lfgFrame.DragonUI_NativeSize then
        local w, h = lfgFrame:GetSize()
        if w and w > 0 then
            lfgFrame.DragonUI_NativeSize = {w, h}
        end
    end
    local nativeW = lfgFrame.DragonUI_NativeSize and lfgFrame.DragonUI_NativeSize[1]
    if nativeW and nativeW > 0 then
        lfgFrame:SetScale(21 / nativeW)
    end

    -- Hook the native button's SetPoint/ClearAllPoints so it stays centered.
    -- Store originals on the frame so ResetMinimapSystem can restore them.
    if not lfgFrame.DragonUI_OrigSetPoint then
        lfgFrame.DragonUI_OrigSetPoint = lfgFrame.SetPoint
    end
    if not lfgFrame.DragonUI_OrigClearAllPoints then
        lfgFrame.DragonUI_OrigClearAllPoints = lfgFrame.ClearAllPoints
    end
    lfgFrame.SetPoint = function(self, ...)
        if self.DragonUI_Locked then return end
        self.DragonUI_OrigSetPoint(self, ...)
    end
    lfgFrame.ClearAllPoints = function(self)
        if self.DragonUI_Locked then return end
        self.DragonUI_OrigClearAllPoints(self)
    end
    lfgFrame.DragonUI_Locked = true

    self.lfgFrameAnchored = true
end
...
function MinimapModule:ApplyMinimapSystem()
    ...
    if minimapModuleConfig and minimapModuleConfig.sexymap_mode == "sexymap" then
        -- Even in SexyMap mode, anchor the queue eye to the minimap and keep it native.
...
function MinimapModule:RestoreMinimapSystem()
    ...
    if lfgFrame and not MicromenuOwnsLFGEye() then
        ...
        if lfgFrame.DragonUI_OrigClearAllPoints then
            lfgFrame.ClearAllPoints = lfgFrame.DragonUI_OrigClearAllPoints
        end
        if lfgFrame.DragonUI_OriginalScale then
            lfgFrame:SetScale(lfgFrame.DragonUI_OriginalScale)
        end
        if lfgFrame.DragonUI_OriginalParent then
            lfgFrame:SetParent(lfgFrame.DragonUI_OriginalParent)
        end
        if lfgFrame.DragonUI_OriginalPoints then
            lfgFrame:ClearAllPoints()
            for _, pointData in ipairs(lfgFrame.DragonUI_OriginalPoints) do
    ...
    local holder = _G["DragonUI_LFGHolder"]
    if holder then
        if holder.DragonUI_OrigSetPoint then
            holder.SetPoint = holder.DragonUI_OrigSetPoint
        end
        if holder.DragonUI_OrigClearAllPoints then
            holder.ClearAllPoints = holder.DragonUI_OrigClearAllPoints
        end
        holder:Hide()
    end
    self.lfgFrameAnchored = false
...
function MinimapModule:InitializeMinimapSystem()
    ...
    -- Dungeon Eye: anchor and style the native queue eye to the minimap.
    self:RegisterLFGEditorFrame()
...
function MinimapModule:UpdateSettings()
    ...
    if Minimap then
        ...
        if playerArrowSize then
            ...
            if Minimap.SetPlayerTextureWidth then
...
local function SetShowClockCVar(enabled)
    -- MoP 5.4.8 does not have a "showClock" CVar.
    -- Clock visibility is controlled directly via TimeManagerClockButton:Show/Hide
    -- in ApplyClockAndZoneLayout. This helper is kept only for API compatibility.
end
