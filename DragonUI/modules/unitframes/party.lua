...
local function IsCompactRaidFrameAddonLoaded()
    ...
    -- MoP uses the standard CVar API.
    if GetCVarBool then
...
local function ShouldHidePartyFramesInRaid()
    return (addon.GetNumRaidMembers() > 0) and IsHidePartyInRaidEnabled()
end
...
local function SetupPartyHooks()
    ...
    -- Additional hook for party member updates (compatible with 3.3.5a/MoP)
...
connectionFrame:RegisterEvent("UNIT_CONNECTION")
connectionFrame:SetScript("OnEvent", function(self, event, unit)
    if not unit or not string.match(unit, "^party%d$") then return end
    local index = tonumber(string.match(unit, "^party(%d)$"))
    local frame = index and _G['PartyMemberFrame' .. index]
    if frame then
...
recoveryFrame:RegisterEvent("GROUP_ROSTER_UPDATE")      -- Party/raid composition changes (MoP)
...
recoveryFrame:RegisterEvent("UNIT_HEALTH_FREQUENT")     -- High-frequency health updates (MoP)
recoveryFrame:RegisterEvent("UNIT_MAXHEALTH")           -- Max health changes
...
recoveryFrame:SetScript("OnEvent", function(self, event, unit)
    ...
    -- For UNIT_HEALTH / UNIT_HEALTH_FREQUENT, only process party units
    if event == "UNIT_HEALTH" or event == "UNIT_HEALTH_FREQUENT" or event == "UNIT_MAXHEALTH" then
