...
function TextSystem.CreateDualTextElements(parentFrame, barFrame, prefix, layer, font)
    ...
    local function StyleText(fs)
        -- Ensure a valid font; fallback to GameFontNormal if the template produced none
        local fontPath, originalSize, flags = fs:GetFont()
        if not fontPath or not originalSize or originalSize <= 0 then
            local fallbackFontObject = _G.GameFontNormal
            if fallbackFontObject then
                fs:SetFontObject(fallbackFontObject)
                fontPath, originalSize, flags = fs:GetFont()
            end
        end
        if fontPath and originalSize then
            fs:SetFont(fontPath, math.max(originalSize + 1, 10), flags)
        end
        -- Make sure text is visible (white, fully opaque, high draw layer)
...
function TextSystem.UpdateDualText(parentFrame, prefix, formattedText, textFormat, shouldShow)
    ...
    if textFormat == TextSystem.TEXT_FORMATS.both and type(formattedText) == "table" then
        ...
        ShowText(leftText, formattedText.left)
        ShowText(rightText, formattedText.right)
    else
        ...
        ShowText(centerText, formattedText)
    end
end
...
function TextSystem.SetupFrameTextSystem(frameType, unit, parentFrame, healthBar, manaBar, prefix)
    ...
    -- Force an immediate update so text is visible right after setup
    updateCallback()
