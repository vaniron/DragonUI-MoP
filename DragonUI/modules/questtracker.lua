...
-- Defensive guard: this MoP client still uses WatchFrame, but some builds may use ObjectiveTrackerFrame.
-- If WatchFrame is missing, abort loading this module; a true ObjectiveTrackerFrame client needs a full rewrite.
if not WatchFrame then
    if ObjectiveTrackerFrame and addon.Print then
        addon:Print("DragonUI QuestTracker: ObjectiveTrackerFrame detected; WatchFrame module disabled.")
    end
    return
end
...
local function GetTrackedQuestsCount()
    ...
    -- MoP auto-accept/complete popups also occupy tracker space.
    if GetNumAutoQuestPopUps then
        local ok, numPopups = pcall(GetNumAutoQuestPopUps)
        if ok and numPopups then
            count = count + numPopups
        end
    end
    return count
end
...
function QuestTrackerModule:Initialize()
    ...
    if WatchFrame then
        local point, relativeTo, relativePoint, x, y = WatchFrame:GetPoint(1)
        if point then
...
local function CollectWatchFrameLinkButtons()
    local found = {}
    local lines = WatchFrameLines
    if not lines then return found end
    -- MoP creates anonymous buttons from WatchFrameLinkButtonTemplate.
    for _, child in pairs({lines:GetChildren()}) do
        if child and child:IsObjectType("Button") and child.startLine and child.lastLine then
...
local function SyncQuestLineHoverButtons()
    ...
    local found = CollectWatchFrameLinkButtons()
    local maxItems = WATCHFRAME_NUM_ITEMS or 40
    for i = 1, maxItems do
...
local function SyncQuestTrackerHitRect()
    ...
    -- MoP scenarios also occupy tracker space.
    if WatchFrameScenarioFrame and WatchFrameScenarioFrame:IsShown() then
        local b = WatchFrameScenarioFrame:GetBottom()
        if b and (not contentBottom or b < contentBottom) then
            contentBottom = b
        end
    end
...
local function InstallQuestTrackerHooks()
    ...
    if WatchFrame_Update then
        hooksecurefunc('WatchFrame_Update', function()
            ...
            if not ok and addon.debug then
                addon:Print("QuestTracker WatchFrame_Update hook error:", err)
            end
        end)
    end
    ...
    if UIParent_ManageFramePositions then
        hooksecurefunc("UIParent_ManageFramePositions", function()
            ...
            if not ok and addon.debug then
                addon:Print("QuestTracker UIParent_ManageFramePositions hook error:", err)
            end
        end)
    end

    -- GetItemCooldown returns nil mid-swap; suppress the resulting nil cooldown error.
    if WatchFrameItem_UpdateCooldown then
        hooksecurefunc("WatchFrameItem_UpdateCooldown", function(self)
            if self and self.Cooldown and not self.Cooldown:IsShown() then
                -- no-op observer; the original already hides on nil cooldown
            end
        end)
    end
