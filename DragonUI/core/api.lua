...
-- ============================================================================
-- MoP 5.4.8 API COMPATIBILITY HELPERS
-- ============================================================================

-- Vehicle action button count global removed in MoP; keep a sane fallback.
if not VEHICLE_MAX_ACTIONBUTTONS then
    VEHICLE_MAX_ACTIONBUTTONS = 6
end

function addon.GetNumPartyMembers()
    return (IsInGroup() and not IsInRaid()) and GetNumGroupMembers() or 0
end

function addon.GetNumRaidMembers()
...
function addon.CreateUIFrame(width, height, frameName)
    ...
    -- Keep overlay hidden until editor mode is toggled on.
    frame:Hide()
    return frame
end
...
function addon.ShowUIFrame(frame)
    ...
    -- Keep the overlay hidden so it does not intercept unit-frame clicks.
    frame:Hide()
end

-- Hide a UI frame (enable editor mode for this frame)
function addon.HideUIFrame(frame, exclude)
    -- Make the overlay visible and interactive while editing.
    frame:Show()
